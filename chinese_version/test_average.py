# Last updated in English
from pentatonic_melody import run_global_grover
from comparison_experiment import run_quantum_grover_experiment

def test_avg_grover():
    print("=== 多次运行Grover测量平均 ===")
    
    legal_ratios = []
    for i in range(10):
        counts = run_global_grover(1024)
        legal = sum(counts.get(code, 0) for code in ["000", "001", "010", "011", "100"])
        total = sum(counts.values())
        ratio = legal / total * 100
        legal_ratios.append(ratio)
        print(f"第{i+1}次: {ratio:.1f}%")
    
    avg = sum(legal_ratios) / len(legal_ratios)
    std = (sum((x - avg) ** 2 for x in legal_ratios) / len(legal_ratios)) ** 0.5
    
    print(f"\n平均: {avg:.1f}%")
    print(f"标准差: {std:.2f}%")
    print(f"范围: {min(legal_ratios):.1f}% - {max(legal_ratios):.1f}%")

def test_avg_comparison():
    print("\n=== 对照实验平均 ===")
    
    ko2_ratios = []
    for i in range(10):
        _, ratio = run_quantum_grover_experiment(2)
        ko2_ratios.append(ratio)
        print(f"Ko=2, 第{i+1}次: {ratio:.1f}%")
    
    avg_ko2 = sum(ko2_ratios) / len(ko2_ratios)
    std_ko2 = (sum((x - avg_ko2) ** 2 for x in ko2_ratios) / len(ko2_ratios)) ** 0.5
    
    print(f"\nKo=2 平均: {avg_ko2:.1f}%")
    print(f"Ko=2 标准差: {std_ko2:.2f}%")
    print(f"Ko=2 范围: {min(ko2_ratios):.1f}% - {max(ko2_ratios):.1f}%")

test_avg_grover()
test_avg_comparison()
